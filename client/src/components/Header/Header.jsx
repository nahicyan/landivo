            ...
            <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
              <SheetTrigger asChild>
                <Button variant="ghost" size="icon">
                  {mobileMenuOpen ? <X className="w-6 h-6 text-[#324c48]" /> : <Menu className="w-6 h-6 text-[#324c48]" />}
                </Button>
              </SheetTrigger>
              <SheetContent side="left" className="bg-[#FDF8F2] border-r border-[#e3a04f]">
                <div className="space-y-4">
                  {/* Properties dropdown for mobile */}
                  <div className="space-y-2">
                    <p className="text-lg font-medium text-[#324c48]">Properties</p>
                    {areaItems.map((item) => (
                      <Link key={item.path} to={item.path} className="block pl-4 text-base text-[#324c48] hover:text-[#D4A017]" onClick={() => setMobileMenuOpen(false)}>
                        {item.name}
                      </Link>
                    ))}
                  </div>
                  ...
                  {[
                    ...
                    <Link key={item.name} to={item.path} className="block text-lg font-medium text-[#324c48] hover:text-[#D4A017]" onClick={() => setMobileMenuOpen(false)}>
                      {item.name}
                    </Link>
                  ))}
                    ...
                    <>
                      <div className="pt-2 mt-4 border-t border-[#e3a04f]/40">
                        ...
                        <PermissionGuard permission={PERMISSIONS.ACCESS_ADMIN}>
                          <Link to="/admin" className="block text-lg font-medium text-[#324c48] hover:text-[#D4A017]" onClick={() => setMobileMenuOpen(false)}>
                            Admin Dashboard
                          </Link>
                        </PermissionGuard>

                        <PermissionGuard permission={PERMISSIONS.WRITE_PROPERTIES}>
                          <Link to="/admin/add-property" className="block text-lg font-medium text-[#324c48] hover:text-[#D4A017] mt-2" onClick={() => setMobileMenuOpen(false)}>
                            Add Property
                          </Link>
                        </PermissionGuard>
                      </div>
                  ...
                  {isLoading ? (
                    ...
                    <Button onClick={handleLogin} className="w-full bg-[#3f4f24] text-white hover:bg-[#2c3b18]">
                      Login / Sign Up
                    </Button>
                  ) : (
                    <Button onClick={handleLogout} className="w-full bg-[#324c48] text-white hover:bg-[#253838]">
                      Logout
                    </Button>
                  )}
                </div>
