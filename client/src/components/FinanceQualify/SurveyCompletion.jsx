...
export default function SurveyCompletion({ surveyData }) {
  ...
  const translations = {
    en: {
      ...
      review:
        "We'll review your application and contact you shortly to discuss next steps for purchasing this property.",
      sorry: "We're Sorry",
      notQualified:
        "You Currently Do Not Qualify For Our Seller Finance Program",
      helpMessage: "However, we may still be able to help!",
      nextSteps: "Here's what you can do next:",
      step1:
        "Provide additional documents (such as proof of income, bank statements, or credit report)",
      step2: "Speak with our team to explore possible solutions",
      reviewMessage:
        "We're happy to review your situation and see if we can find a way to move forward.",
    ...
    es: {
      ...
      review:
        "Revisaremos su solicitud y nos comunicaremos con usted en breve para hablar sobre los próximos pasos para la compra de esta propiedad.",
      sorry: "Lo sentimos",
      notQualified:
        "Actualmente no califica para nuestro programa de financiación del vendedor",
      helpMessage: "Sin embargo, ¡aún podemos ayudarle!",
      nextSteps: "Esto es lo que puede hacer a continuación:",
      step1:
        "Proporcionar documentos adicionales (como comprobante de ingresos, estados de cuenta bancarios o informe crediticio)",
      step2: "Hable con nuestro equipo para explorar posibles soluciones",
      reviewMessage:
        "Estaremos encantados de revisar su situación y ver si podemos encontrar una manera de avanzar.",
  ...
  const handleDownloadResults = () => {
    alert(
      "This feature would generate a PDF qualification report showing the user's qualification status and details."
    );
  };
  ...
  const handleCallTeam = () => {
    if (systemPhone) {
      window.location.href = `tel:+1${cleanPhone(systemPhone)}`;
    }
  };
  ...
  const handleSendDocuments = () => {
    window.location.href =
      "mailto:info@landivo.com?subject=Additional Documents for Qualification";
  };
              ...
              {disqualified ? (
                <div className="space-y-6">
                  ...
                  <div className="bg-white rounded-lg shadow-md border border-gray-200 p-6 mb-6 max-w-md mx-auto">
                    <h3 className="font-semibold text-[#3f4f24] mb-4 text-left">
                      {t.nextSteps}
                    </h3>
