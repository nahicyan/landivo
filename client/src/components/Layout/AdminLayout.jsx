...
export default function AdminLayout() {
  ...
  const menuCategories = [
    {
      name: "Overview",
      items: [{ text: "Dashboard", icon: <Home className="h-5 w-5" />, path: "/admin" }],
    },
    {
      name: "Management",
      items: [
    ...
    {
      name: "Communications",
      items: [{ text: "Email Lists", icon: <ListChecks className="h-5 w-5" />, path: "/admin/email-lists" }],
    },
    {
      name: "Applications",
  ...
  return (
    <div className="bg-[#FDF8F2] text-[#333] min-h-screen flex flex-col">
      ...
      <div className="flex flex-grow">
        ...
        <aside
          className={cn(
            "h-[calc(100vh-80px)] top-20 border-r border-[#324c48]/20 bg-gradient-to-b from-[#fcfaf6] to-[#f1f6ea] shadow-lg transition-all duration-300 ease-in-out fixed z-10",
            collapsed ? "w-16" : "w-64"
          )}>
          <div className="flex items-center justify-between p-2 bg-gradient-to-r from-[#324c48]/15 to-[#324c48]/5">
            {!collapsed && (
              <div className="flex items-center space-x-3">
                ...
                <h2 className="font-bold text-lg text-[#324c48] tracking-tight">Landivo</h2>
              </div>
            )}
            <Button
              variant="ghost"
              ...
              className={cn("h-8 w-8 rounded-full bg-white/80 shadow-md hover:bg-[#324c48]/10 transition-all hover:shadow-lg", collapsed && "mx-auto")}>
              {collapsed ? <ChevronRight className="h-4 w-4 text-[#324c48]" /> : <ChevronLeft className="h-4 w-4 text-[#324c48]" />}
            </Button>
          </div>
            ...
            <div className="py-2">
              {menuCategories.map((category, index) => (
                <div key={index} className="mb-2">
                  {!collapsed && <h3 className="text-xs font-bold uppercase tracking-wider text-[#324c48]/60 ml-4 mb-1">{category.name}</h3>}
                    ...
                    return (
                      <Link
                        ...
                        className={cn(
                          ...
                          isActive ? "bg-gradient-to-r from-[#546930]/25 to-[#546930]/15 text-[#324c48] shadow-sm" : "hover:bg-[#324c48]/10 text-[#324c48]/80"
                        )}>
                        <div
                          className={cn(
                            "flex items-center justify-center rounded-lg transition-all",
                            collapsed ? "h-8 w-8 mx-auto" : "h-7 w-7 mr-3",
                            isActive ? "bg-gradient-to-br from-[#546930] to-[#324c48] text-white shadow-sm" : "bg-[#324c48]/10 text-[#324c48]"
                          )}>
                          {item.icon}
                        </div>

                        {!collapsed && <span className={cn("transition-all", isActive ? "font-bold" : "")}>{item.text}</span>}
                      </Link>
                    );
                  })}

                  {!collapsed && index < menuCategories.length - 1 && <Separator className="my-2 mx-4 opacity-30" />}
                </div>
              ))}
            </div>
          </ScrollArea>
        ...
        <main className={cn("flex-grow bg-gradient-to-b from-white to-[#f4f7ee]/50 min-h-[calc(100vh-160px)] transition-all duration-300", collapsed ? "ml-16 p-6" : "ml-64 p-8")}>
