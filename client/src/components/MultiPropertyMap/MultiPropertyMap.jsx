...
export default function MultiPropertyMap({ properties = [] }) {
  ...
  const createInfoWindowContent = (property) => {
    ...
    const imageUrl = images.length > 0 ? `${serverURL}/${images[0]}` : `${serverURL}/default-image.jpg`;
    ...
    const getMonthlyPayment = () => {
      ...
      const payments = [property.monthlyPaymentOne, property.monthlyPaymentTwo, property.monthlyPaymentThree].filter((payment) => payment && !isNaN(payment));
    ...
    const displayAddress = getDisplayAddress(property.streetAddress, property.toggleObscure, showAddress, property.county);

    // Create DOM element
    const content = document.createElement("div");
    content.style.cssText = "width: 280px; font-family: system-ui, -apple-system, sans-serif; overflow: hidden; padding: 0; border-radius: 0; background: white;";

    // Image container with tags
    const imageContainer = document.createElement("div");
    imageContainer.style.cssText = "position: relative; width: 100%; height: 180px; overflow: hidden; border-radius: 0;";
    ...
    if (isSold) {
      ...
      soldBadge.style.cssText =
        "position: absolute; top: 8px; right: 8px; background: linear-gradient(to right, #ef4444, #dc2626); color: white; font-size: 12px; font-weight: 600; padding: 4px 12px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);";
    ...
    link.target = "_blank";
    link.style.cssText =
      "display: inline-block; width: 100%; text-align: center; padding: 8px 16px; background: #324c48; color: white; text-decoration: none; border-radius: 8px; font-size: 14px; font-weight: 500;";
  ...
  useEffect(() => {
    ...
    const initializeMap = () => {
      ...
      style.textContent = `
        ...
        .gm-style-iw-d { overflow: hidden !important; margin-top: -4px !important; }
      `;
      ...
      class PriceLabel extends google.maps.OverlayView {
        ...
        onAdd() {
          ...
          label.onmouseover = function () {
            this.style.background = "#253936";
          };
          label.onmouseout = function () {
            this.style.background = "#324c48";
          };
      ...
      const centerLat = validProperties.reduce((sum, p) => sum + parseFloat(p.latitude), 0) / validProperties.length;
      const centerLng = validProperties.reduce((sum, p) => sum + parseFloat(p.longitude), 0) / validProperties.length;
      ...
      validProperties.forEach((property) => {
        ...
        const price = property.askingPrice ? `$${(parseFloat(property.askingPrice) / 1000).toFixed(0)}k` : "N/A";
