...
export default function SanAntonioProperty() {
  ...
  // Apply search filter to SanAntonio properties
  const filteredSanAntonioProperties = SanAntonioProperties.filter(
    (property) => {
      const query = areaQuery.toLowerCase();
      if (!query) return true; // If no query, return all SanAntonio properties
  ...
  // 1. If filtered SanAntonio properties exist, show them
  // 2. If no SanAntonio properties (or no matching filtered ones), show filtered all properties
