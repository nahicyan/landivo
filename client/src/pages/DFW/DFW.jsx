...
export default function DFWProperty() {
  ...
  useEffect(() => {
    ...
    const fetchDFWRow = async () => {
      ...
      try {
        ...
        const rows = await getPropertyRows("DFW");

        if (Array.isArray(rows) && rows.length > 0) {
          const dfwRow = rows.find((row) => row.rowType === "DFW");
  ...
  const nonFeaturedDFWProperties = dfwProperties.filter((property) => !featuredPropertyIds.includes(property.id));
    ...
    <div className="bg-[#FDF8F2] min-h-screen py-12 text-[#4b5b4d]">
      <div className="max-w-screen-xl mx-auto px-4">
        ...
        <div className="mb-10 text-center">
          <h1 className="text-3xl sm:text-4xl font-bold mb-4">{hasDFWProperties || hasFeaturedProperties ? "Properties in Dallas Fort Worth" : "All Properties"}</h1>
          ...
          <SearchAreaWithTracking query={areaQuery} setQuery={setAreaQuery} placeholder="Search in this area" area="DFW" filteredData={filteredDFWProperties} />
        </div>
        ...
        {!hasDFWProperties && areaQuery && (
          <div className="text-center py-8">
            <p className="text-lg text-gray-600 mb-4">No Dallas Fort Worth properties match "{areaQuery}".</p>
        ...
        {!hasFeaturedProperties && !hasDFWProperties && !showFallback && (
          <div className="text-center py-12">
            <p className="text-lg text-gray-600 mb-4">{areaQuery ? `No properties found matching "${areaQuery}".` : "Sorry! We sold through everything in Dallas Fort Worth!"}</p>
          </div>
        )}
