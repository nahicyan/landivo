...
useEffect(() => {
  ...
  const fetchAustinRow = async () => {
    ...
    try {
      ...
      console.log("Austin row API response:", response.data);
      ...
      if (Array.isArray(response.data) && response.data.length > 0) {
        ...
        if (austinRow && Array.isArray(austinRow.displayOrder) && austinRow.displayOrder.length > 0) {
          ...
          console.log("Property IDs in Austin row:", orderedIds);
          
          // Create a map for faster property lookup
          const propertiesMap = new Map(data.map(p => [p.id, p]));
          
          // Get properties in order, filtering for featured properties in Austin
          const orderedProperties = orderedIds
            .map(id => propertiesMap.get(id))
            .filter(property => property && property.featured === "Featured" && property.area === "Austin");
          ...
          console.log("No displayOrder array in Austin row");
          setFeaturedProperties([]);
          setFeaturedPropertyIds([]);
        }
      } else {
        console.log("No Austin row found in response");
        setFeaturedProperties([]);
        setFeaturedPropertyIds([]);
      }
    } catch (error) {
  ...
  // Apply search filter to non-featured Austin properties
  ...
  // Apply search filter to all properties EXCEPT featured Austin properties
  const filteredAllProperties = data.filter(property => {
    // Skip featured Austin properties
  ...
  const displayProperties = filteredAustinProperties.length > 0
  ...
  const headerText = featuredProperties.length > 0 
    ? "Other Properties" 
    : (filteredAustinProperties.length > 0 ? "Properties in Austin" : "All Properties");

  const subtitleText = filteredAustinProperties.length > 0
    ? "Browse through properties available in Austin."
    : nonFeaturedAustinProperties.length > 0 
      ...
      <div className="max-w-screen-xl mx-auto px-4">
        ...
        <div className="mb-10 text-center">
          <h1 className="text-3xl sm:text-4xl font-bold mb-4">
            {featuredProperties.length > 0 ? "Properties in Austin" : headerText}
          </h1>
        ...
        {loadingFeatured ? (
          ...
          <div className="mb-12">
            <div className="text-center mb-6">
              <h2 className="text-2xl sm:text-3xl font-bold mb-2">Featured Properties in Austin</h2>
              <div className="mx-auto w-16 h-1 bg-[#D4A017] mb-3"></div>
              <p className="text-[#324c48]/80">Our top picks in the Austin area</p>
            </div>
