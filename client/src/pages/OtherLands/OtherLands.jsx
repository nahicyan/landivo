...
useEffect(() => {
  ...
  const fetchOtherLandsRow = async () => {
    ...
    try {
      ...
      console.log("OtherLands row API response:", response.data);
      
      // Check if we have an array response with at least one row
      if (Array.isArray(response.data) && response.data.length > 0) {
        // Find the OtherLands row
        ...
        if (otherLandsRow && Array.isArray(otherLandsRow.displayOrder) && otherLandsRow.displayOrder.length > 0) {
          ...
          console.log("Property IDs in OtherLands row:", orderedIds);
          
          // Create a map for faster property lookup
          const propertiesMap = new Map(data.map(p => [p.id, p]));
          
          // Get properties in order, filtering for featured properties in Other Areas
          const orderedProperties = orderedIds
            .map(id => propertiesMap.get(id))
            .filter(property => property && property.featured === "Featured" && property.area === "Other Areas");
          ...
          console.log("No displayOrder array in OtherLands row");
          setFeaturedProperties([]);
          setFeaturedPropertyIds([]);
        }
      } else {
        console.log("No OtherLands row found in response");
        setFeaturedProperties([]);
        setFeaturedPropertyIds([]);
      }
    } catch (error) {
  ...
  // Apply search filter to non-featured Other Areas properties
  ...
  // Apply search filter to all properties EXCEPT featured Other Areas properties
  const filteredAllProperties = data.filter(property => {
    // Skip featured Other Areas properties
  ...
  const displayProperties = filteredOtherAreasProperties.length > 0
    ? filteredOtherAreasProperties
    : filteredAllProperties;

  // Set header text and subtitle
  const headerText = featuredProperties.length > 0 
    ? "Other Properties" 
    : (filteredOtherAreasProperties.length > 0 ? "Properties in Other Areas" : "All Properties");

  const subtitleText = filteredOtherAreasProperties.length > 0
    ? "Browse through properties available in Other Areas."
    : nonFeaturedOtherAreasProperties.length > 0 
      ...
      <div className="max-w-screen-xl mx-auto px-4">
        ...
        <div className="mb-10 text-center">
          <h1 className="text-3xl sm:text-4xl font-bold mb-4">
            {featuredProperties.length > 0 ? "Properties in Other Areas" : headerText}
          </h1>
        ...
        {/* Featured Properties in Other Areas */}
        {loadingFeatured ? (
          ...
          <div className="mb-12">
            <div className="text-center mb-6">
              <h2 className="text-2xl sm:text-3xl font-bold mb-2">Featured Properties in Other Areas</h2>
              <div className="mx-auto w-16 h-1 bg-[#D4A017] mb-3"></div>
              <p className="text-[#324c48]/80">Our top picks in Other Areas</p>
            </div>
